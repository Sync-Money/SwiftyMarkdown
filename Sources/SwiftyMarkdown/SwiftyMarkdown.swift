...
*/
@objc open class BasicStyles : NSObject {
...
@objc open class LineStyles : NSObject {
    internal init(fontName: String? = nil, color: UIColor? = nil, fontSize: CGFloat? = nil, fontStyle: FontStyle? = nil, alignment: NSTextAlignment? = nil, lineSpacing: CGFloat? = nil, paragraphSpacing: CGFloat? = nil) {
    ...
    static var shared = LineStyles(color: UIColor.black, fontStyle: .normal, alignment: .left, lineSpacing: 0.0, paragraphSpacing: 0.0)
    public var fontName : String?
    #if os(macOS)
    public var color: NSColor?
    #else
...
extension SwiftyMarkdown {
    ...
    func attributedStringFor( tokens : [Token], in line : SwiftyLine ) -> NSAttributedString {
        ...
        var lineProperties: LineStyles? {
            didSet {
                if oldValue?.alignment != nil && lineProperties?.alignment == nil {
                    lineProperties?.alignment = oldValue?.alignment
                }
                if oldValue?.lineSpacing != nil && lineProperties?.lineSpacing == nil {
                    lineProperties?.lineSpacing = oldValue?.lineSpacing
                }
                if oldValue?.paragraphSpacing != nil && lineProperties?.paragraphSpacing == nil {
                    lineProperties?.paragraphSpacing = oldValue?.paragraphSpacing
                }
            }
        }
        
    
        guard let markdownLineStyles = line.lineStyle as? [MarkdownLineStyle] else {
            preconditionFailure("The passed line style is not a valid Markdown Line Style")
        }
        
        var listItem = self.bullet
        
        markdownLineStyles.reversed().forEach { markdownLineStyle in
        ...
        paragraphStyle.alignment = lineProperties?.alignment ?? LineStyles.shared.alignment!
        paragraphStyle.lineSpacing = lineProperties?.lineSpacing ?? LineStyles.shared.lineSpacing!
        paragraphStyle.paragraphSpacing = lineProperties?.paragraphSpacing ?? LineStyles.shared.paragraphSpacing!
        attributes[.paragraphStyle] = paragraphStyle
